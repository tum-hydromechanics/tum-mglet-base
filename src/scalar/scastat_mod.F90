...
CONTAINS
    SUBROUTINE init_scastat()
        ...
        DO l = 1, nsca
            ...
            ! TT_AVG
            CALL register_statfield(TRIM(scalar(l)%name)//TRIM(scalar(l)%name)&
            //"_AVG", comp_sqr_avg) 

            ! TxTx_AVG
            CALL register_statfield(TRIM(scalar(l)%name)//"x"//TRIM(scalar(l)%name)&
            //"x"//"_AVG", comp_txtx_avg)
            ! TyTy_AVG
            CALL register_statfield(TRIM(scalar(l)%name)//"y"//TRIM(scalar(l)%name)&
            //"y"//"_AVG", comp_txtx_avg)
            ! TzTz_AVG
            CALL register_statfield(TRIM(scalar(l)%name)//"z"//TRIM(scalar(l)%name)&
            //"z"//"_AVG", comp_txtx_avg)

            ! UT_AVG
            CALL register_statfield("U"//TRIM(scalar(l)%name)//"_AVG", comp_ut_avg)
            ! VT_AVG
            CALL register_statfield("V"//TRIM(scalar(l)%name)//"_AVG", comp_ut_avg)
            ! WT_AVG
            CALL register_statfield("W"//TRIM(scalar(l)%name)//"_AVG", comp_ut_avg)
    ...
    SUBROUTINE  comp_txtx_avg(field, name, dt)
        ...
        !Local Variables
        TYPE(field_t) :: tx_f ! It can be the derivative of a scalar in any direction
        ...
        CALL field%init(name, istag=istag, jstag=jstag, kstag=kstag, &
            units=units)
        CALL tx_f%init(name, istag=istag, jstag=jstag, kstag=kstag, &
            units=units_tx)
    ...
    SUBROUTINE comp_ut_avg(field, name, dt)
        ...
        ! Multiplication of U and the scalar (T) 
    ...
    SUBROUTINE comp_utt_avg(field, name, dt)
       ! Subroutine arguments
        ...
        ! Multiplication considering staggeredness
